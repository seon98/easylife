...
def with_quality(program: dict):
    ...
    fields = {
        ...
        "지원 대상": bool(program.get("target")), "지원 내용": bool(program.get("benefit")),
        "신청 방법": bool(program.get("application_method")), "문의처": bool(program.get("contact")),
        "제출 문서": bool(program.get("documents_json") and program.get("documents_json") != "[]"),
    }
    completed = sum(fields.values())
    review = program.get("review_status") or "PENDING"
    quality = "VERIFIED" if review == "APPROVED" else "EXTRACTED" if program.get("detail_collected_at") else "BASIC"
    return {**program, "data_quality": {"score": round(completed / len(fields) * 100), "status": quality, "fields": fields}}
...
def search_collected(q: str = "", region: str | None = None, category: str | None = None, status: str | None = None, limit: int = 100):
    ...
    for token in q.split():
        clauses.append("(title LIKE ? OR ministry LIKE ? OR organization LIKE ? OR summary LIKE ? OR target LIKE ? OR benefit LIKE ?)")
...
def get_collected(source_id: str):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        conn.row_factory = sqlite3.Row
        row = conn.execute("SELECT * FROM programs WHERE source_id = ?", (source_id,)).fetchone()
...
def review_queue(limit: int = 100):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        conn.row_factory = sqlite3.Row
        return [dict(row) for row in conn.execute("SELECT * FROM programs WHERE detail_collected_at IS NOT NULL ORDER BY CASE review_status WHEN 'PENDING' THEN 0 ELSE 1 END, detail_collected_at DESC LIMIT ?", (min(limit, 200),))]

def set_review_status(source_id: str, status: str) -> bool:
    if status not in {"PENDING", "APPROVED", "NEEDS_EDIT"}: return False
    with sqlite3.connect(DB_PATH) as conn:
        result = conn.execute("UPDATE programs SET review_status=? WHERE source_id=?", (status, source_id))
        conn.commit()
