...
"""기업마당 공개 웹페이지의 목록과 상세정보를 SQLite로 동기화한다."""
...
USER_AGENT = (
    "EasyLifeSupportSearch/1.0 (+public-data collector; contact: admin@example.com)"
)
# 기관 서버에 출처가 분명한 요청만 보내고 CLI에서 최소 요청 간격을 강제한다.


def clean(value: str) -> str:
    """HTML에서 추출한 연속 공백과 줄바꿈을 하나의 공백으로 정리한다."""
    return " ".join(value.split())


def region_from(title: str, ministry: str) -> str:
    """제목의 [지역] 표기를 우선 사용하고 없으면 소관기관에서 지역을 추론한다."""
    match = re.match(r"\[([^]]+)]", title)
    if match:
        return match.group(1).split("·")[0]
    if ministry.endswith(
        ("특별시", "광역시", "특별자치시", "특별자치도")
    ) or ministry in ("경기도", "강원도", "충청북도", "충청남도", "전라북도", "전라남도", "경상북도", "경상남도"):
        return (
            ministry.replace("특별자치도", "")
            .replace("특별시", "")
            .replace("광역시", "")
            .replace("도", "")
        )
...
def status_from(period: str) -> str:
    """신청 시작·종료일과 오늘 날짜를 비교해 화면용 접수 상태를 계산한다."""
...
def page_rows(
    session: requests.Session, page: int, ended: bool = False
) -> tuple[list[dict], int]:
    """기업마당 목록 한 페이지를 공통 프로그램 필드로 변환한다."""
    response = session.get(
        LIST_URL,
        params={"rows": 15, "cpage": page, "schEndAt": "Y" if ended else "N"},
        timeout=20,
    )
    ...
    for tr in soup.select(".table_Type_1 tbody tr"):
        ...
        period, ministry, organization = (
            clean(cells[i].get_text(" ", strip=True)) for i in (3, 4, 5)
        )
        rows.append(
            {
                "source_id": source_id,
                ...
                "published_at": clean(cells[6].get_text(" ", strip=True)),
                "region": region_from(title, ministry),
                "status": status_from(period),
                "official_url": urljoin(BASE_URL, href),
            }
        )
    last_link = soup.select_one("a[title='마지막페이지']")
    last_page = (
        int(parse_qs(urlparse(last_link["href"]).query).get("cpage", [page])[0])
        if last_link
        else page
    )
...
def collect_closed(max_pages: int = 5, delay: float = 0.25) -> int:
    """홈의 마감 소식에 사용할 최근 종료 공고를 제한된 페이지 수만 수집한다."""
    session = requests.Session()
    session.headers.update(
        {"User-Agent": USER_AGENT, "Accept-Language": "ko-KR,ko;q=0.9"}
    )
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        for page in range(1, max_pages + 1):
            ...
            for item in rows:
                conn.execute(
                    """INSERT INTO programs(source_id,title,category,period,ministry,organization,published_at,region,status,official_url,source_name,source_type,collected_at,raw_source_url) VALUES(:source_id,:title,:category,:period,:ministry,:organization,:published_at,:region,:status,:official_url,'기업마당','OFFICIAL_WEB',:collected_at,:official_url) ON CONFLICT(source_id) DO UPDATE SET status=excluded.status,period=excluded.period,collected_at=excluded.collected_at""",
                    {**item, "collected_at": now},
                )
            conn.commit()
            total += len(rows)
...
def labeled_value(soup: BeautifulSoup, label: str) -> str:
    """기업마당 상세 화면에서 주어진 라벨의 본문 값을 찾는다."""
...
def infer_target(summary: str) -> str:
    ...
    for line in summary.splitlines():
        ...
        if any(
            word in normalized
            for word in ("대상", "기업", "소상공인", "청년", "근로자", "창업자", "거주")
        ):
...
def infer_benefit(summary: str) -> str:
    ...
    for line in summary.splitlines():
        ...
        if any(
            word in normalized
            for word in ("지원", "최대", "한도", "만원", "억원", "보조", "융자")
        ):
            candidates.append(normalized)
    return " ".join(candidates[-3:])[:1000]
...
def detail_record(session: requests.Session, row: dict) -> dict:
    ...
    summary_node = next(
        (
            item.select_one(".txt")
            for item in soup.select(".view_cont > ul > li")
            if item.select_one(".s_title")
            and clean(item.select_one(".s_title").get_text(" ", strip=True))
            == "사업개요"
        ),
        None,
    )
    summary = (
        "\n".join(
            clean(line)
            for line in summary_node.get_text("\n", strip=True).splitlines()
            if clean(line)
        )
        if summary_node
        else ""
    ...
    file_names = [
        clean(node.get("title", "") or node.get_text(" ", strip=True))
        for node in soup.select("#iframe[title], a[onclick*='fileLoad']")
    ]
    documents = [name for name in dict.fromkeys(file_names) if name]
    record = {
        "summary": summary,
        "target": infer_target(summary),
        "benefit": infer_benefit(summary),
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        rows = [
            ...
            for row in conn.execute(
                f"SELECT * FROM programs WHERE {condition} ORDER BY published_at DESC LIMIT ?",
                (limit,),
            )
        ]
        completed = 0
        for row in rows:
            try:
                ...
                snapshot = json.dumps(
                    {**row, **detail}, ensure_ascii=False, sort_keys=True
                )
                conn.execute(
                    "INSERT OR IGNORE INTO program_versions(source_id,content_hash,snapshot_json,detected_at) VALUES(?,?,?,?)",
                    (row["source_id"], detail["content_hash"], snapshot, now),
                )
                conn.execute(
                    """UPDATE programs SET summary=:summary,target=:target,benefit=:benefit,application_method=:application_method,contact=:contact,documents_json=:documents_json,content_hash=:content_hash,detail_collected_at=:now,review_status='PENDING' WHERE source_id=:source_id""",
                    {**detail, "now": now, "source_id": row["source_id"]},
                )
                conn.commit()
                completed += 1
...
def enrich_program(source_id: str, refresh: bool = False) -> dict | None:
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        conn.execute(
            "INSERT OR IGNORE INTO program_versions(source_id,content_hash,snapshot_json,detected_at) VALUES(?,?,?,?)",
            (source_id, detail["content_hash"], snapshot, now),
        )
        conn.execute(
            """UPDATE programs SET summary=:summary,target=:target,benefit=:benefit,application_method=:application_method,contact=:contact,documents_json=:documents_json,content_hash=:content_hash,detail_collected_at=:now,review_status='PENDING' WHERE source_id=:source_id""",
            {**detail, "now": now, "source_id": source_id},
        )
        conn.commit()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="기업마당의 공개 지원사업 목록을 API 키 없이 수집합니다."
    )
    parser.add_argument(
        "--max-pages", type=int, default=None, help="테스트용 최대 페이지 수"
    )
    parser.add_argument(
        "--delay", type=float, default=0.25, help="페이지 요청 간격(초)"
    )
    parser.add_argument(
        "--details", type=int, default=0, help="상세 내용을 보완할 최대 공고 수"
    )
    parser.add_argument(
        "--refresh-details",
        action="store_true",
        help="이미 수집한 상세 내용도 다시 확인",
    )
    parser.add_argument(
        "--closed-pages", type=int, default=0, help="최근 마감 공고를 가져올 페이지 수"
    )
    args = parser.parse_args()
    print(f"목록 완료: {collect(args.max_pages, max(args.delay, 0.2))}개")
    if args.details:
        print(
            f"상세 완료: {collect_details(args.details, max(args.delay, 0.3), args.refresh_details)}개"
        )
    if args.closed_pages:
        print(
            f"마감 공고 완료: {collect_closed(args.closed_pages, max(args.delay, 0.2))}개"
        )
